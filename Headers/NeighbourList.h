void RenewList(){ // Initialise Neighbour List
    ...
    Config.W0W1 = Config.W[0]*Config.W[1]; 
    
    Config.w[0] = Config.W[0]/Config.l[0];
    Config.w[1] = Config.W[1]/Config.l[1];
    Config.w[2] = Config.W[2]/Config.l[2];
	
    Config.head.resize(Config.W0W1*Config.W[2]); 
    Config.usedCell.resize(Config.W0W1*Config.W[2],false); 
	...
	if( Config.Ns == 0){
		...
		for( int i=0; i<Config.part.size(); i++){ 
			...
			int k = sx[1] + Config.W[0]*sy[1] + Config.W0W1*sz[1]; 
			...
			k = sxN[1] + Config.W[0]*syN[1] + Config.W0W1*szN[1]; 
